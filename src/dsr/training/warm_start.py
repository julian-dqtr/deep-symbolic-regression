...
warm_start.py
=============
Warm-start by dataset similarity — lightweight meta-learning.

Motivation
----------
Standard training initialises the policy with random weights for every new
task. But the DeepSets encoder in the policy already produces a 64-dim
embedding that captures the statistical structure of the dataset (scale,
variance, correlations between variables). Two tasks with similar embeddings
likely share symbolic structure — e.g. feynman_I_10_7 (m₀/sqrt(1-v²/c²))
and feynman_I_15_10 (m₀v/sqrt(1-v²/c²)) both involve the Lorentz factor.

Warm-starting from the most similar solved checkpoint should:
  1. Reduce the number of episodes needed to converge
  2. Improve final NMSE on structurally related tasks
  3. Demonstrate that the DeepSets embedding encodes meaningful task structure

This is a lightweight form of meta-learning inspired by:
  - MAML (Finn et al., 2017) — model-agnostic meta-learning
  - Transfer learning by embedding similarity

Implementation
--------------
1. For each checkpoint in checkpoint_dir, compute the DeepSets embedding
   of its stored dataset (X, y).
2. For the new task, compute its embedding.
3. Find the checkpoint with minimum cosine distance to the new task.
4. Load that checkpoint's policy weights as the starting point.
5. Train normally from there.

The key insight: we reuse the DeepSets encoder that is already trained
as part of the policy — no extra model needed.

Usage (standalone)
------------------
python -m dsr.training.warm_start \
    --suite pmlb_feynman_subset \
    --checkpoint_dir checkpoints \
    --num_episodes 3000 \
    --compare_cold_start   # also run cold start for comparison

Usage (from Python)
-------------------
from dsr.training.warm_start import find_most_similar_checkpoint, warm_start_trainer
