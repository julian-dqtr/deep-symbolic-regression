...
====================
Multi-task training: one shared policy trained across all Feynman tasks.

Motivation
----------
The standard Trainer trains one policy per task — each starting from scratch.
The DeepSets encoder is designed to condition the policy on a specific dataset,
but its potential for generalisation across tasks is never exploited.

Multi-task training lets the policy:
  1. Generalise across expression structures (reuse +, *, sqrt patterns)
  2. Learn a shared prior over symbolic expressions
  3. Converge faster on new tasks via the shared LSTM backbone

Design
------
At each update step:
  1. Sample a random task from the task pool
  2. Set the dataset embedding for that task (DeepSets encoder)
  3. Collect a batch of episodes for that task
  4. Update the shared policy with RSPG

All tasks share the same Grammar (same vocab_size = max_num_variables).
Each task gets its own memory buffer (TopKMemory) so replay is task-specific.

This mirrors the multi-task RL literature (e.g. MT-SAC, PEARL) where a
shared backbone is conditioned on a task embedding at inference time.

Comparison
----------
After training, we evaluate the shared policy on each task and compare
against single-task baselines trained with the same total episode budget.

Usage
-----
python -m dsr.training.multitask_trainer \
...
------
results/
  multitask_<timestamp>.csv
  plots/
    multitask_vs_single_barplot.png
    multitask_convergence.png
"""
