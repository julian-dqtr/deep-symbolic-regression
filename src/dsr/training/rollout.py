...
rollout.py
==========
Episode collection utilities for the RSPG symbolic regression agent.

Three functions are provided:

collect_episode         — sequential, interacts with the real SymbolicRegressionEnv.
                          rewards[] is populated from env.step() directly.
                          Use for debugging or single-episode evaluation.

recompute_episode       — teacher-forcing: replays a fixed token sequence through
                          the policy to recompute gradients for memory replay.
                          rewards[] is populated from env.step(); final_reward
                          reflects the true environment reward for that sequence.

collect_batched_episodes — fast vectorised batch collection using pure PyTorch.
                           Does NOT call env.step() — rewards[] is intentionally
                           left empty and must be filled by the caller (Trainer)
                           after expression evaluation. This is the only function
                           used during training and is ~10× faster than the
                           sequential version for batch_size >= 64.

Interface contract
------------------
All three functions return a list (or single dict) with the same keys:
    tokens        : List[str]
    action_ids    : List[int]
    log_probs     : List[Tensor]      — differentiable
    entropies     : List[Tensor]      — differentiable
    values        : List[Tensor]      — differentiable
    observations  : List[dict]        — detached snapshots for PPO
    rewards       : List[float]       — 0.0 intermediate, final_reward at end
                                        (empty for collect_batched_episodes)
    final_reward  : float             — terminal reward (0.0 until filled by caller
                                        for collect_batched_episodes)
"""
