...
mcts.py
=======
Monte Carlo Tree Search (MCTS) guided by the trained RSPG policy.

Motivation
----------
During training, the policy samples stochastically — it may know a good
expression but fail to produce it reliably. Beam search improves on this
but is purely greedy. MCTS provides the best of both worlds:

  - Uses the policy as a prior (like AlphaZero's policy network)
  - Explores promising branches via UCB (Upper Confidence Bound)
  - Backtracks when branches fail — unlike beam search
  - Finds expressions that neither sampling nor beam search would find

Algorithm (AlphaZero-style, adapted for symbolic expressions)
-------------------------------------------------------------
Each tree node represents a partial expression state:
  - tokens    : prefix tokens chosen so far
  - pending   : open slots remaining (0 = complete expression)

At each MCTS iteration:
  1. SELECT    — traverse the tree using UCB1 until a leaf
  2. EXPAND    — add all valid children using the policy prior
  3. EVALUATE  — complete the expression via a policy rollout,
                 compute NMSE reward on the dataset
  4. BACKPROP  — update visit counts and value estimates up the tree

Final answer: the child of the root with the highest mean value,
then evaluate its complete expression on (X, y).

Key difference from beam search
--------------------------------
Beam search keeps the top-k partial expressions at each step.
MCTS keeps a tree and revisits nodes — it can escape local optima
by exploring branches that initially looked unpromising.
...
python -m dsr.training.mcts \
    ...
    --num_simulations 200 \
    --sampling_csv results/true_vs_recovered_<timestamp>.csv

Usage (from Python)
-------------------
from dsr.training.mcts import mcts_search
...
print(results["best_nmse"], results["best_expr"])
"""
